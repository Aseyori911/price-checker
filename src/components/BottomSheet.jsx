    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '12px 0 4px' }}>
          <div style={{ width: '40px', height: '4px', borderRadius: '99px', background: theme.cardBorder }} />
        </div>

        {/* HEADER */}
        <div style={{
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          padding: '8px 20px 16px', borderBottom: `1px solid ${theme.cardBorder}`
        }}>
          <div>
            <h3 style={{ fontSize: '15px', fontWeight: 700, color: theme.text, margin: '0 0 2px' }}>
              💬 Comments
            </h3>
            <p style={{ fontSize: '12px', color: theme.textMuted, margin: 0 }}>
              {comments.length} {comments.length === 1 ? 'comment' : 'comments'} {post ? `on ${post.product}` : ''}
            </p>
          </div>
          <button
            onClick={handleClose}
            style={{
              ...
              background: theme.pill, border: 'none', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '16px', color: theme.textMuted, fontWeight: 700,
            }}
          >×</button>
        </div>
            ...
            comments.map((c, i) => (
              <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '11px', fontWeight: 800, color: '#0a0a0f',
                }}>
                  {typeof c === 'string' ? c.charAt(0).toUpperCase() : 'U'}
                </div>
                <div style={{
                  flex: 1, background: theme.pill, borderRadius: '0 14px 14px 14px',
                  padding: '10px 14px',
                }}>
                  <p style={{ fontSize: '11px', fontWeight: 700, color: theme.accent, margin: '0 0 4px' }}>
                    Community Member
                  </p>
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              fontSize: '14px', background: theme.input,
